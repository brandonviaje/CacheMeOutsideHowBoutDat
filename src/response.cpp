...
{
    *header = out.size();
    buffer_append_u32(out, 0);
}

std::size_t response_size(Buffer &out, size_t header)
{
...
{
    std::size_t msg_size{response_size(out, header)}; // exclude header
    ...
    uint32_t len = htonl(static_cast<uint32_t>(msg_size));
